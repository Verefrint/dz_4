...
export class CustomTransfer extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save CustomTransfer entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type CustomTransfer must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("CustomTransfer", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): CustomTransfer | null {
    return changetype<CustomTransfer | null>(
      store.get_in_block("CustomTransfer", id.toHexString()),
    );
  }

  static load(id: Bytes): CustomTransfer | null {
    return changetype<CustomTransfer | null>(
      store.get("CustomTransfer", id.toHexString()),
    );
  }
